...
const App = {
    config: {
        STORAGE_KEYS: { entries: 'workEntries_v6_weekdays', settings: 'appSettings_v6_weekdays' },
        persianMonths: ['فروردین','اردیبهشت','خرداد','تیر','مرداد','شهریور','مهر','آبان','آذر','دی','بهمن','اسفند'],
        persianWeekdays: ['یکشنبه', 'دوشنبه', 'سه‌شنبه', 'چهارشنبه', 'پنجشنبه', 'جمعه', 'شنبه'],
        defaultSettings: {
            standardStartTime: '07:00', standardEndTime: '15:00', dailyWorkHours: 8,
            thursdayWorkHours: 4, autoFillEndTime: true, 
    ...
    init: function() {
        ...
        this.UI.updateShamsiDateDisplay(this.DOM.workDateInput.value, this.DOM.workDateShamsiDisplay, true);
        this.UI.updateShamsiDateDisplay(this.DOM.reportFromDateInput.value, this.DOM.reportFromDateShamsiDisplay, false);
        this.UI.updateShamsiDateDisplay(this.DOM.reportToDateInput.value, this.DOM.reportToDateShamsiDisplay, false);
    },

    Events: {
        setupEventListeners: function() { 
            App.DOM.tabButtons.forEach(btn => btn.addEventListener('click', () => App.UI.showTab(btn.dataset.tab, btn)));
            App.DOM.workDateInput.addEventListener('change', (e) => { App.UI.updateShamsiDateDisplay(e.target.value, App.DOM.workDateShamsiDisplay, true); App.UI.checkAndDisplayHolidayStatus(e.target.value); App.Utils.debounce(App.UI.autoFillEndTime, App.config.debounceDelay)(); });
            App.DOM.reportFromDateInput.addEventListener('change', (e) => App.UI.updateShamsiDateDisplay(e.target.value, App.DOM.reportFromDateShamsiDisplay, false));
            App.DOM.reportToDateInput.addEventListener('change', (e) => App.UI.updateShamsiDateDisplay(e.target.value, App.DOM.reportToDateShamsiDisplay, false));
    ...
    UI: { 
        ...
        updateShamsiDateDisplay: function(gregorianDateStr, displayElement, showWeekday) { 
            if(displayElement) {
                let text = App.JalaliDate.gregorianToJalaaliString(gregorianDateStr);
                if (showWeekday && gregorianDateStr) {
                    const date = new Date(gregorianDateStr);
                    const weekday = App.config.persianWeekdays[date.getDay()];
                    text += ` ( ${weekday} )`;
                }
                displayElement.textContent = text;
            }
        },
        checkAndDisplayHolidayStatus: function(gregorianDateStr) { 
            const sD=App.JalaliDate.gregorianToJalaaliString(gregorianDateStr);
            const iH=(App.state.appSettings.holidays || []).includes(sD);
            App.DOM.holidayIndicator.textContent=iH?'تعطیل رسمی':'';
            
            const date = new Date(gregorianDateStr);
            const weekday = date.getDay(); // Sunday = 0, ..., Thursday = 4, Friday = 5
            App.DOM.isThursdayWorkCheckbox.checked = (weekday === 4);
            App.DOM.isFridayWorkCheckbox.checked = (weekday === 5);
        },
        ...
        resetEntryForm: function(clearDescription = false) { App.state.editingEntryId=null;App.DOM.editingEntryIdInput.value='';App.DOM.entryFormTitle.textContent='ثبت اطلاعات';App.DOM.saveWorkEntryBtn.textContent='ثبت اطلاعات';App.DOM.saveWorkEntryBtn.classList.remove('btn-warning');const t=new Date().toISOString().split('T')[0];App.DOM.workDateInput.value=t;App.UI.updateShamsiDateDisplay(t,App.DOM.workDateShamsiDisplay, true);App.UI.checkAndDisplayHolidayStatus(t);App.DOM.isDailyLeaveCheckbox.checked=false;App.UI.toggleTimeInputsBasedOnDailyLeave();App.DOM.breakTimeInput.value='0';App.DOM.hourlyLeaveInput.value='0';if(clearDescription)App.DOM.workDescriptionInput.value='';App.DOM.workDateInput.focus();},
        autoFillEndTime: function() { if(!App.state.appSettings.autoFillEndTime||App.state.editingEntryId||App.DOM.isDailyLeaveCheckbox.checked)return;const sDS=App.DOM.workDateInput.value;const sTS=App.DOM.startTimeInput.value;const isThursday = App.DOM.isThursdayWorkCheckbox.checked;const dailyHours=isThursday?parseFloat(App.state.appSettings.thursdayWorkHours):parseFloat(App.state.appSettings.dailyWorkHours);if(sDS&&sTS&&!isNaN(dailyHours)&&dailyHours>0){try{const sDT=new Date(`${sDS}T${sTS}`);if(isNaN(sDT.getTime())){App.DOM.endTimeInput.value='';return;}const eT=new Date(sDT.getTime()+dailyHours*60*60*1000);App.DOM.endTimeInput.value=`${eT.getHours().toString().padStart(2,'0')}:${eT.getMinutes().toString().padStart(2,'0')}`; }catch(e){App.DOM.endTimeInput.value='';}}else App.DOM.endTimeInput.value='';},
        populateFormForEdit: function(entryId) { const e=App.state.workEntries.find(i=>i.id===entryId);if(!e){App.UI.showNotification('رکورد یافت نشد.','error');return;}App.state.editingEntryId=e.id;App.DOM.editingEntryIdInput.value=e.id;App.DOM.entryFormTitle.textContent='ویرایش اطلاعات';App.DOM.saveWorkEntryBtn.textContent='ذخیره تغییرات';App.DOM.saveWorkEntryBtn.classList.add('btn-warning');App.DOM.workDateInput.value=e.date;App.UI.updateShamsiDateDisplay(e.date,App.DOM.workDateShamsiDisplay, true);App.UI.checkAndDisplayHolidayStatus(e.date);App.DOM.isDailyLeaveCheckbox.checked=e.isDailyLeave||false;App.DOM.isManualHolidayCheckbox.checked=e.isManualHoliday||false;App.DOM.isThursdayWorkCheckbox.checked=e.isThursdayWork||false;App.DOM.isFridayWorkCheckbox.checked=e.isFridayWork||false;App.UI.toggleTimeInputsBasedOnDailyLeave();if(!e.isDailyLeave){App.DOM.startTimeInput.value=e.startTime;App.DOM.endTimeInput.value=e.endTime;App.DOM.breakTimeInput.value=e.breakTime;App.DOM.hourlyLeaveInput.value=e.hourlyLeaveMinutes||0;}App.DOM.workDescriptionInput.value=e.description;App.UI.showTab('entry');App.DOM.workDateInput.focus();},
        displayWorkEntries: function() {
            ...
            container.innerHTML = filteredEntries.map(entry => {
                ...
                if (entry.isFridayWork) { statusBadges += '<span class="entry-status-badge badge-friday">جمعه کاری</span>'; }
                if (entry.isThursdayWork) { statusBadges += '<span class="entry-status-badge badge-thursday">پنجشنبه کاری</span>'; }
        ...
        updateDashboard: function() {
            ...
            `<div class="entry-item"><div class="entry-header"><span class="entry-date">${App.JalaliDate.gregorianToJalaaliString(e.date)} ${e.isPredefinedHoliday||e.isManualHoliday||e.isFridayWork ? '(تعطیل/جمعه کاری)' : (e.isThursdayWork ? '(پنجشنبه کاری)' : '')}</span><span>${e.isDailyLeave?'مرخصی‌روزانه':App.Utils.formatHours(e.totalHours)+' ساعت'}</span></div>
    ...
    Data: { 
        saveWorkEntry: function() {
            ...
            const isThursdayWork = App.DOM.isThursdayWorkCheckbox.checked; const isFridayWork = App.DOM.isFridayWorkCheckbox.checked;
            ...
            const entryData = {id:App.state.editingEntryId||App.Utils.generateId(),date:gregorianDate,startTime:isDailyLeave?'':App.DOM.startTimeInput.value,endTime:isDailyLeave?'':App.DOM.endTimeInput.value,breakTime:isDailyLeave?0:(parseInt(App.DOM.breakTimeInput.value)||0),hourlyLeaveMinutes:isDailyLeave?0:(parseInt(App.DOM.hourlyLeaveInput.value)||0),description:App.DOM.workDescriptionInput.value.trim(),isDailyLeave:isDailyLeave,isPredefinedHoliday:isPredefinedHoliday,isManualHoliday:isManualHoliday, isThursdayWork: isThursdayWork, isFridayWork: isFridayWork};
            ...
            const effectiveHoliday=isPredefinedHoliday||isManualHoliday||isFridayWork;
            ...
            let tWM=(e-s)/(1e3*60)-entryData.breakTime-entryData.hourlyLeaveMinutes;if(tWM<0)tWM=0;entryData.totalHours=parseFloat((tWM/60).toFixed(2));
            const dailyWorkHoursSetting = isThursdayWork ? parseFloat(App.state.appSettings.thursdayWorkHours) : parseFloat(App.state.appSettings.dailyWorkHours);
            if(effectiveHoliday){entryData.regularHours=0;entryData.overtimeHours=entryData.totalHours;}else{entryData.regularHours=Math.min(entryData.totalHours,dailyWorkHoursSetting);entryData.overtimeHours=Math.max(0,entryData.totalHours-dailyWorkHoursSetting);}}catch(e){App.UI.showNotification('خطا در محاسبه ساعات.','error');return;}}
    ...
    Settings: {
        ...
        save: function() { App.state.appSettings.standardStartTime=App.DOM.settingStdStartTimeInput.value;App.state.appSettings.standardEndTime=App.DOM.settingStdEndTimeInput.value;App.state.appSettings.dailyWorkHours=parseFloat(App.DOM.settingDailyWorkHoursInput.value)||8;App.state.appSettings.thursdayWorkHours=parseFloat(App.DOM.settingThursdayWorkHoursInput.value)||4;App.state.appSettings.holidays=[...(App.state.tempHolidays||[])].sort();App.state.appSettings.autoFillEndTime=App.DOM.settingAutoFillEndTimeCheckbox.checked;try{localStorage.setItem(App.config.STORAGE_KEYS.settings,JSON.stringify(App.state.appSettings));App.UI.showNotification('تنظیمات ذخیره شد.','success');App.UI.updateDashboard();App.UI.resetEntryForm();this.renderHolidayList();}catch(e){App.UI.showNotification('خطا ذخیره تنظیمات.','error');}},
        populateForm: function() { App.DOM.settingStdStartTimeInput.value=App.state.appSettings.standardStartTime;App.DOM.settingStdEndTimeInput.value=App.state.appSettings.standardEndTime;App.DOM.settingDailyWorkHoursInput.value=App.state.appSettings.dailyWorkHours;App.DOM.settingThursdayWorkHoursInput.value=App.state.appSettings.thursdayWorkHours;App.DOM.settingAutoFillEndTimeCheckbox.checked=App.state.appSettings.autoFillEndTime;this.renderHolidayList();},
    ...
    Export: { 
        ...
        createBackup: function() {
            try {
                ...
                App.Utils.downloadFile("data:text/json;charset=utf-8," + encodeURIComponent(jsonContent), `backup_shamsi_v6_${new Date().toISOString().split('T')[0]}.json`);
